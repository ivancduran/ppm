...
case '-l': case 'list':
	if(requesting=='http'){
		var req_new = http.request(url, function(res) {
			res.setEncoding('utf8');
			res.on('data', function (data) {
				var jObj = JSON.parse(data);
...
// VERSION ****************************************
case '-v': case 'version':
	console.log('Version: 0.0.6'.green);
break;
...
var fn = {
	...
	update : function()
	{
		...
		if (process.argv[2]==='update' && fs.existsSync(cache+'pkg.json') ){
			requesting = 'http';
